...
async function runSeeds() {
    ...
    try {
        ...
        const existingAdmin = await (0, database_1.default)("users").where({ email: adminEmail }).first();
        let adminId = (0, uuid_1.v4)();
        if (!existingAdmin) {
            ...
            await (0, database_1.default)("users").insert({
                ...
                email_verified_at: database_1.default.fn.now()
            });
        ...
        // 2. Seed Categories
        const categories = [
            { id: (0, uuid_1.v4)(), slug: "frontend", name_uz: "Frontend Dasturlash", name_en: "Frontend Development", name_ru: "Frontend Разработка" },
            { id: (0, uuid_1.v4)(), slug: "backend", name_uz: "Backend Dasturlash", name_en: "Backend Development", name_ru: "Backend Разработка" },
            { id: (0, uuid_1.v4)(), slug: "system-design", name_uz: "Tizim Dizayni", name_en: "System Design", name_ru: "Системный Дизайн" },
            { id: (0, uuid_1.v4)(), slug: "cybersecurity", name_uz: "Kiberxavfsizlik", name_en: "Cybersecurity", name_ru: "Кибербезопасность" },
            { id: (0, uuid_1.v4)(), slug: "devops", name_uz: "DevOps va Bulutlar", name_en: "DevOps & Cloud", name_ru: "DevOps и Облака" }
        ];
        const categoryMap = {};
        for (const cat of categories) {
            const existing = await (0, database_1.default)("categories").where({ slug: cat.slug }).first();
            if (!existing) {
                await (0, database_1.default)("categories").insert(cat);
        ...
        // 3. Seed Tags
        const tags = [
            { id: (0, uuid_1.v4)(), name: "vue" },
            { id: (0, uuid_1.v4)(), name: "react" },
            { id: (0, uuid_1.v4)(), name: "nodejs" },
            { id: (0, uuid_1.v4)(), name: "express" },
            { id: (0, uuid_1.v4)(), name: "postgresql" },
            { id: (0, uuid_1.v4)(), name: "docker" },
            { id: (0, uuid_1.v4)(), name: "linux" },
            { id: (0, uuid_1.v4)(), name: "nginx" },
            { id: (0, uuid_1.v4)(), name: "security" },
            { id: (0, uuid_1.v4)(), name: "jwt" }
        ];
        const tagMap = {};
        for (const tag of tags) {
            const existing = await (0, database_1.default)("tags").where({ name: tag.name }).first();
            if (!existing) {
                await (0, database_1.default)("tags").insert(tag);
...
`,
        ...
        for (const art of articles) {
            const existing = await (0, database_1.default)("blog_articles").where({ slug: art.slug }).first();
            if (!existing) {
                await (0, database_1.default)("blog_articles").insert(art);
                // Link tags
                ...
                for (const tid of associatedTags) {
                    if (tid) {
                        await (0, database_1.default)("blog_article_tags").insert({
                            article_id: art.id,
                            tag_id: tid
                        });
                    }
